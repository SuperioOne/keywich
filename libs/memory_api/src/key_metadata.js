import {parse_KeyOptions} from "./parsers/index.js";

/**
 * @type {WeakMap<import("@keywich/api").KeyMetadataItem, string>}
 */
const IMAGE_STORE = new WeakMap();

/**
 * @type {import("@keywich/api").KeyMetadataItem[]}
 */
const MEMORY_STORE = [
  {
    charset: "A..Za..z",
...
/**
 * @type {import("@keywich/api").KeyMetadataRPC}
 */
const module = {
  unpin_key(id) {
  ...
  get_key_collection(filter) {
    ...
    if (filter) {
      ...
      /**
       * @return {boolean}
       * @param {import("@keywich/api").KeyMetadataItem} item
       */
      const filterFn = (item) => {
        if (filter.tag) {
