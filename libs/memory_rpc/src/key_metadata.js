import {parse_KeyOptions} from "@keywich/rpc/parsers";

/**
 * @type {WeakMap<import("@keywich/rpc").KeyMetadataItem, string>}
 */
const IMAGE_STORE = new WeakMap();

/**
 * @type {import("@keywich/rpc").KeyMetadataItem[]}
 */
const MEMORY_STORE = [
  {
    charset: "A..Za..z",
...
/**
 * @type {import("@keywich/rpc").KeyMetadataRPC}
 */
const module = {
  unpin_key(id) {
  ...
  get_key_collection(filter) {
    ...
    if (filter) {
      ...
      /**
       * @return {boolean}
       * @param {import("@keywich/rpc").KeyMetadataItem} item
       */
      const filterFn = (item) => {
        if (filter.tag) {
