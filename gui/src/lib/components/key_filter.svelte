<script lang="ts">
  ...
  function on_window_click(event: Event) {
    ...
    const is_in_container =
      input_container && input_container.contains(event.target as Node);
    const is_in_menu =
      menu_element && menu_element.contains(event.target as Node);
  ...
  function create_token_node(token: TokenType): HTMLSpanElement {
    ...
    switch (token.type) {
      ...
      case "tag": {
        ...
        node.className =
          "p-1.5 text-sm rounded-md bg-surface-active-token ml-1";
...
<div class="flex flex-row w-full input overflow-hidden">
  {#if is_focused || !is_null_or_empty(query)}
    <div class="flex flex-row justify-between" bind:this={input_container}>
  ...
  >
    <div>
      <span class="font-bold"
        >{$i18nStore.get_key("i18:/filter/options", "Filter options:")}</span
      >
      <ul class="p-1">
    ...
    {#if $filterHistoryStore.length > 0}
      ...
      <div>
        <div class="flex flex-row justify-between items-center">
          <span class="font-bold"
            >{$i18nStore.get_key("i18:/filter/history", "History")}</span
          >
          <button
