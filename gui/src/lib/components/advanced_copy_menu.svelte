<script lang="ts">
  ...
  import { is_error_response } from "../utils";
  import { onDestroy } from "svelte";
  import type { PasswordOutputType } from "../api/types";
  ...
  type DisplayData =
    | {
        state: "completed";
        display_data: string;
        type: "Json" | "Base64" | "Text" | "UriEncoded";
      }
    | {
        state: "completed";
        display_data: string;
      ...
      switch (output_type) {
        case "Qr": {
          ...
          display = {
            ...
            state: "completed",
            display_data: URL.createObjectURL(
              new Blob([encoded_svg], { type: "image/svg+xml" }),
            ),
          };
          break;
        }
  ...
  async function save_qr(data: Uint8Array) {
    try {
      await Api.save_file(data);
      toast_store.trigger_success(
        $i18nStore.get_key(
          "i18:/actions/advanced-copy/qr-saved",
          "File saved.",
        ),
      );
    } catch (err) {
      ...
      if (is_error_response(err)) {
        toast_store.trigger_error(
          $i18nStore.get_key(`i18:/errors/${err.code}`, "Save Failed."),
        );
      } else {
        toast_store.trigger_error(
          $i18nStore.get_key(
            "i18:/actions/advanced-copy/unknown-error",
            "Unexpected error.",
          ),
        );
      }
...
{#if $modal_store[0]}
  <div class="card p-6 flex flex-col items-center gap-5 w-full sm:w-modal-slim">
    ...
    {:else}
      ...
      <div class="grid grid-cols-2 gap-2 w-full">
        ...
        >
          <LinkIcon size={48} />
          <span class="font-mono text-sm text-center w-full !m-0"
            >URI Encoded</span
          >
        </button>
      </div>
