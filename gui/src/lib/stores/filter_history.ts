...
import { create_debouncer, is_null_or_empty } from "../utils";
...
function loader(): FilterHistoryItem[] {
  try {
    ...
    Log.warn(
      " Filter history format is incorrect. App storage database might be corrupted or altered.",
    );
    return [];
  }
}
...
const write_scheduler = create_debouncer(
    ...
    localStorage.setItem(STORAGE_KEY, JSON.stringify(items)),
  {
    ...
    onError: Log.error,
    onSuccess: () =>
      Log.debug("Filter history persisted to the local storage."),
  },
);
