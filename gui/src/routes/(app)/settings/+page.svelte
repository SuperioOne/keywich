<script lang="ts">
  ...
  import BackupSettings from "./settings_backup.svelte";
...
<div class="flex flex-row flex-wrap sm:flex-nowrap gap-2 w-full">
  <div
    class="w-full sm:w-fit p-4 rounded-none border-b-[1px] border-surface-600 sm:border-r-[1px] sm:border-b-[0px]"
  >
    <ListBox active="variant-filled-secondary" spacing="space-y-3">
      ...
      <ListBoxItem bind:group={selection} name="medium" value="backup">
        {$i18nStore.get_key("i18:/settings/nav/backup", "Backup/Restore")}
      </ListBoxItem>
    </ListBox>
  </div>
