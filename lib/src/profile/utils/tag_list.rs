...
impl TagList {
  ...
  pub fn difference(&self, other: &TagList) -> Self {
    let diff: HashSet<Box<str>> = self.internal.difference(&other.internal).cloned().collect();

    TagList { internal: diff }
  }

  pub fn union(&self, other: &TagList) -> Self {
    let union: HashSet<Box<str>> = self.internal.union(&other.internal).cloned().collect();
